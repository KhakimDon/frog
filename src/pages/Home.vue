...
export default {
  ...
  mounted() { 
    ...
    this.$el.addEventListener('click', this.closeBurger)
  ...
  methods: {
    closeBurger(){
        if(event.target.id == "burger" || event.target.id == "openburger"){

        }else{
          console.log('asda');
          document.querySelector("#burgermenu").classList.remove("activeMenu")
        }
    },
    reload() {
    ...
    <div id="left" class="h-[100%] w-[50%] bg-[#43A047] flex-col p-[30px] text-[#fff]">
      <div id="logo_wrp" class="flex justify-between items-center">
        ...
        <div class="h-[30px] relative w-[200px] bg-[#69B36C] flex items-center">
          ...
          <span id="openburger"  @click="openMenu()" class="w-[160px] text-center cursor-pointer">Уровень {{ +Object.prototype.idx + 1 }}
            из 24
          </span>

          <BurgersLvl id="burgermenu" @reset="reset()" @changeLevel="changeLevel()" :class="{ activeMenu: this.open }" />
      ...
      <div class="h-[100%] w-[100%] flex relative justify-between ">
        ...
        <div id="answer" :style="this.fallback.answer" class="flex z-10 p-[30px] h-[100%] justify-start w-[100%] absolute l-0 t-0">
          <svg v-for="i of this.fallback.frogs" :key="i.id" class="m-[10px]"
            ...
            width="160px" height="160px" viewBox="0 0 160 160" preserveAspectRatio="none" version="1.1">
            <!-- Generator: Sketch 3.4.3 (16618) - http://www.bohemiancoding.com/sketch -->
            <defs>
              <path id="path-1"
